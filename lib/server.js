...
_this.listen = function (port, loaded) {
    ...
    _this.register(port || this.port, (request, response) => {
        ...
        // 去掉第一个问号之后的内容
        let query_simple_index = url_path.indexOf("?");
        if (query_simple_index != -1) {
            url_path = url_path.slice(0, query_simple_index);
        }
        ...
        // 设置已存在格式的contentType，未设置的文件类型不进行设置
        if (Config.sourceContentType.hasOwnProperty(ext)) {
            response.writeHead(200, { "Content-Type": Config.sourceContentType[ext] })
            encode = "binary"
        }
